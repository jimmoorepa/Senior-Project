...
app.get('/view/(:prodid)', function(req, res, next) {
	  ...
	  var prodQuery = "SELECT * FROM product WHERE id = ?";
		  conn.query(prodQuery, [req.params.id], function(err, rows, fields) {
		  //conn.query('SELECT * FROM product where Available = 1', 
			...
			if (err) {
				...
				console.log("Connected to Product table");
				res.render('products/list', {
					title: 'Product Details', 
					//id: rows[0].id,
					data: rows 
...
// SHOW EDIT USER FORM
app.get('/edit/(:prodid)', function(req, res, next){
	req.getConnection(function(error, conn) {
		conn.query('SELECT * FROM product WHERE id = ?', [req.params.id], function(err, rows, fields) {
			if(err) throw err
